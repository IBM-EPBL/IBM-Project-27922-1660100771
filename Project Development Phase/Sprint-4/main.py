...
def login():
    if request.method == 'POST':
        ...
        result = con.fetchall()
        if result:
            return '<h1>Login Successfully <a href="/dash">Go to dashboard</a></h1>'
...
def dashboard():
    if request.method == 'POST':
        ...
        f.save(filepath)
        msg='The file name of uploaded file is: {}'.format(f.filename)
        return '<h2>'+msg+'<br><a href="/dash/action">Get Graph</a></h2>'
    else:
        return render_template('dash.html',message="")

@app.route('/dash/action', methods=['GET', 'POST'])
def dashboardAction():
    if request.method == 'POST':
        variable = request.form['variable']
        data = pd.read_csv('static/test.csv')
        plt.plot(data[variable])
        imagepath = os.path.join('static', 'image3' + '.png')
        plt.savefig(imagepath)
        plt.close()
        return render_template('image.html')
    return render_template('dashaction.html')
    ...
    if request.method == 'POST':
        ...
        if psw == pswRepeat:
            ...
            if result:
                ...
                if status:
                    return '<h3>Successfully Registered. Go to <a href="/">Login Page</a></h3>'
                else:
                    return "<h3>Error</h3>"
        else:
